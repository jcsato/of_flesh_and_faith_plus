assassin_poison_03_effect <- inherit("scripts/skills/skill", {
	...
	function getTooltip() {
		...
		local ret = [
			...
			{ id = 11, type = "text", icon = "ui/icons/fatigue.png", text = "Every weapon attack that " + thresholdString + " applies a poison that inflicts [color=" + Const.UI.Color.NegativeValue + "]" + m.FatigueDamageOnHit + "[/color] extra Fatigue" }
			{ id = 12, type = "text", icon = "ui/icons/fatigue.png", text = "Every weapon attack that " + thresholdString + " applies a poison that reduces the target's Fatigue Recovery per turn by [color=" + Const.UI.Color.NegativeValue + "]-15[/color] for 2 turns" }
