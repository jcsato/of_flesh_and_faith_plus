oath_of_righteousness_active_effect <- inherit("scripts/skills/skill", {
	...
	function getTooltip(_withContainer = true) {
		local numSlain = _withContainer ? getContainer().getActor().getFlags().getAsInt(::OFFP.Oathtakers.Flags.Righteousness) : 0;
		local numLeadersSlain = _withContainer ? getContainer().getActor().getFlags().getAsInt(::OFFP.Oathtakers.Flags.RighteousnessLeaders) : 0;
