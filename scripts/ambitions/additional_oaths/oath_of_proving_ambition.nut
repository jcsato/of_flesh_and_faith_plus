oath_of_proving_ambition <- inherit("scripts/ambitions/oaths/oath_ambition",
{
	...
	function create()
	{
		...
		m.SuccessText		= "[img]gfx/ui/events/event_180.png[/img]{There exists disparity between all men, whether by station or by birth or by talent. When the %companyname% took on the Oath of Proving, you knew this disparity would come to the forefront, with some men more fit for combat than others. Despite it all, however, each man ultimately did prove his worth, with those less talented driven to try that much harder next time and those more accustomed eager to see just how far their skills could take them. Indeed, in some small way the men have become closer, and the disparity lessened somewhat in the face of their collective success.\n\nBut enough of this self-measurement - it's time to move on to the next Oath!}";
