	...
	function create()
	{
		...
		m.Screens.push({
							...
							function start(_event) {
								...
								foreach( bro in brothers ) {
									if (bro.getFlags().getAsInt("CursedExplorersRotsActive") > 1) {
		...
		m.Screens.push({
							...
							function start(_event) {
								...
								local brothers = World.getPlayerRoster().getAll();
								foreach( bro in brothers ) {
									if (bro.getFlags().getAsInt("CursedExplorersRotsActive") > 1 && bro.getID() != _event.m.Raider.getID()) {
