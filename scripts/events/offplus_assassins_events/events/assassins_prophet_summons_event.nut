assassins_prophet_summons_event <- inherit("scripts/events/event", {
	...
	function create() {
		...
		m.Screens.push({
			ID			= "A"
			Text		= "{%terrainImage%It has been some time since the Old Man on the Mountain tasked you with becoming a Crownling and exiled you from the guilds. The %companyname% has since grown into a formidable band, regarded as professionals by noble and vizier alike. While the company has faced its share of troubles, by and large mercenary work has proven to be substantially simpler and more profitable than assassinations ever were.\n\nYou ponder what course your life might have taken had this career change happened sooner, what it might have been like outside the auspice of the guilds. Your thoughts wander into progressively more treasonous territory, and you're almost thankful when %randombrother% interrupts your reverie.%SPEECH_ON%There's someone here to see you, captain. A peculiar sort, too.%SPEECH_OFF%}"
			Image		= ""
			List		= [ ]
			Characters	= [ ]
			Options		= [
